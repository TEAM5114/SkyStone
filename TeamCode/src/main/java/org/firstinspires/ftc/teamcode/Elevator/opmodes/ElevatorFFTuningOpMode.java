...
/**
 * Do not tune kStatic and kA if using motor built-in PID
 */
@Config
    ...
    public void runOpMode() throws InterruptedException {
        ...
        if (fitIntercept) {
            telemetry.log().add(Misc.formatInvariant(
                    "kV = %.5f, kStatic = %.5f (R^2 = %.2f)", rampResult.kV, rampResult.kStatic, rampResult.rSquare));
        } else {
            telemetry.log().add(Misc.formatInvariant(
                    "kV = %.5f (R^2 = %.2f)", rampResult.kV, rampResult.rSquare));
        }
