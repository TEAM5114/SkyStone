...
/**
 * Utilities for interacting with Lynx modules
 */
public class LynxModuleUtil {
    ...
    /**
     * Ensure all fo the Lynx modules attached to the robot satisfy the minimum requirement
     * @param hardwareMap
     */
    public static void ensureMinimumFirmwareVersion(HardwareMap hadwareMap){
        Map<String, LynxFirmwareVersion> outdatedModules = new HashMap<>();
        for (LynxModule module : hadwareMap.getAll(LynxModule.class)) {
            LynxFirmwareVersion version = getFirmwareVersion(module);
            if (version == null || version.compareTo(MIN_VERSION) < 0) {
                for (String name : hadwareMap.getNamesOf(module)){
